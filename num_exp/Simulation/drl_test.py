...
def test_scenario_pair(train_scenario_id, test_scenario_id, state_dim, n_a, n_p, ppo_config, models_dir, num_tests_per_env):
    print(f"开始测试: 模型 {train_scenario_id} -> 场景 {test_scenario_id}")
    ...
    try:           
        ...
        if os.path.exists(best_model_path):
            ...
            print(f"警告: 未找到模型 {train_scenario_id} 的最佳模型")
            return train_scenario_id, test_scenario_id, None, None, None
        
    except Exception as e:
        print(f"加载模型 {train_scenario_id} 出现错误: {e}")
        return train_scenario_id, test_scenario_id, None, None, None
    ...
    test_rewards = []
    for test_run in range(num_tests_per_env):
        try:
            test_results = agent.test_model(test_env_fn, num_tests=1, verbose=False)
            test_reward = test_results[0]['total_reward']
            test_rewards.append(test_reward)
        except Exception as e:
            print(f"模型 {train_scenario_id} -> 场景 {test_scenario_id} 测试运行 {test_run + 1} 出现错误: {e}")
            continue
    
    if test_rewards:
        mean_reward = np.mean(test_rewards)
        std_reward = np.std(test_rewards)
        print(f"完成测试: 模型 {train_scenario_id} -> 场景 {test_scenario_id}: 均值={mean_reward:.2f}, 标准差={std_reward:.2f}")
        return train_scenario_id, test_scenario_id, mean_reward, std_reward, test_rewards
    else:
        print(f"失败: 模型 {train_scenario_id} -> 场景 {test_scenario_id} 测试失败")
        return train_scenario_id, test_scenario_id, None, None, None
...
if __name__ == "__main__":
    ...
    all_test_rewards = {
        test_scenario_id: [] for test_scenario_id in scenarios.keys()
    }
    

    pool_args = []
    for train_scenario_id in scenarios.keys():
        for test_scenario_id in scenarios.keys():
            pool_args.append((
                train_scenario_id, test_scenario_id, state_dim, n_a, n_p, 
                ppo_config, models_dir, num_tests_per_env
            ))
            

    num_cores = min(mp.cpu_count(), len(pool_args))
    print(f"使用 {num_cores} 个进程进行并行测试，共 {len(pool_args)} 个测试任务")
    
    with mp.Pool(processes=num_cores) as pool:
        results = pool.starmap(test_scenario_pair, pool_args)
        

    for train_id, test_id, mean_reward, std_reward, test_rewards in results:
        if mean_reward is not None:

            mean_results[train_id - 1, test_id - 1] = mean_reward
            std_results[train_id - 1, test_id - 1] = std_reward
            
            if test_id != train_id:
                all_test_rewards[test_id] += test_rewards
    ...
    scenario_names = [f"Scenario_{i}" for i in scenarios.keys()]
    

    mean_df = pd.DataFrame(mean_results, 
                          index=[f"Train_Scenario_{i}" for i in scenarios.keys()],
                          columns=[f"Test_Scenario_{i}" for i in scenarios.keys()])
    mean_csv_path = os.path.join(results_dir, "drl_results_mean.csv")
    ...
    std_df = pd.DataFrame(std_results,
                         index=[f"Train_Scenario_{i}" for i in scenarios.keys()], 
                         columns=[f"Test_Scenario_{i}" for i in scenarios.keys()])
    std_csv_path = os.path.join(results_dir, "drl_results_std.csv")
    std_df.to_csv(std_csv_path)
    print(f"标准差结果已保存到: {std_csv_path}")

    std_df_off = pd.DataFrame(
        {f"Test_Scenario_{i}": np.std(all_test_rewards[i]) for i in scenarios.keys()},
        index=["STD"]
    )
    
    std_df_off.to_csv(os.path.join(results_dir, "drl_results_std_offpolicy.csv"))
    print(f"离线策略标准差结果已保存到: {os.path.join(results_dir, 'drl_results_std_offpolicy.csv')}")
