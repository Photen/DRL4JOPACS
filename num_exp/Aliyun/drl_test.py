        ...
        for test_scenario_id in scenarios.keys():
            ...
            for test_run in range(num_tests_per_env):
                try:
                    ...
                    if (test_run + 1) % 5 == 0:
                        print(f"  完成测试 {test_run + 1}/{num_tests_per_env}")
            ...
            if test_rewards:
                ...
                if test_scenario_id != train_scenario_id:
                    if test_scenario_id in off_match_set[train_scenario_id]:
                        all_test_rewards[test_scenario_id] += test_rewards
                   


                mean_results[train_scenario_id - 1, test_scenario_id - 1] = mean_reward
                std_results[train_scenario_id - 1, test_scenario_id - 1] = std_reward
                
                print(f"  场景 {train_scenario_id} -> 场景 {test_scenario_id}: "
                      f"均值={mean_reward:.2f}, 标准差={std_reward:.2f}")
            else:
                print(f"  场景 {train_scenario_id} -> 场景 {test_scenario_id}: 测试失败")
