...
class PPOAgent:
    ...
    def get_action(self, state, env, deterministic=False):
        s_tensor = states_to_tensor([state], env)
        with torch.no_grad():
            if deterministic:
                logits, _ = self.model.forward(s_tensor)
                action = torch.argmax(logits, dim=-1)
                a = action // self.n_p
                p = action % self.n_p
            else:
                action, _, _, _, a, p = self.model.get_action_and_value(s_tensor)
        
        a_val = int(a.cpu().numpy()[0])
        p_val = int(p.cpu().numpy()[0])
        return a_val, env.p_list[p_val]

    def evaluate_policy(self, env_fn, num_episodes=20, deterministic=True, base_seed=None, verbose=False):
        rewards = []
        self.model.eval()

        for episode_id in range(num_episodes):
            if base_seed is not None:
                set_global_seed(base_seed + episode_id, deterministic=True)
            ...
            while not done:
                a, p = self.get_action(state, env, deterministic=deterministic)
                done, state, reward = env.step((a, p))
                total_reward += reward
    ...
    def test_model(self, env_fn, num_tests=5, deterministic=True, test_seed=None, verbose=True):
        if verbose:
            mode = "确定性(argmax)" if deterministic else "随机采样"
            print(f"开始进行 {num_tests} 次测试... 模式: {mode}")
        ...
        for test_id in range(num_tests):
            if test_seed is not None:
                set_global_seed(test_seed + test_id, deterministic=True)
...
def train_scenario(
    ...
    seed = int(base_seed) * 1000 + int(train_scenario_id)
    set_global_seed(seed, deterministic=True)

    print(f"\n{'='*60}")
    print(f"开始训练场景 {train_scenario_id}")
    print(f"{'='*60}")
    ...
    try:
        ...
        eval_stats = agent.evaluate_policy(
            env_fn=train_env_fn,
        ...
        print(
            f"场景 {train_scenario_id} 训练完成! 平均训练奖励: {avg_train_reward:.2f} | "
            f"评估均值±标准差: {eval_stats['mean']:.2f}±{eval_stats['std']:.2f}"
        )
        return train_scenario_id, avg_train_reward, eval_stats
    except Exception as e:
        print(f"场景 {train_scenario_id} 训练过程中出现错误: {e}")
        return train_scenario_id, None, None

if __name__ == "__main__":
    import multiprocessing as mp
    parser = argparse.ArgumentParser(description="PPO训练与评估（支持多seed）")
    parser.add_argument('--seed', type=int, default=42, help='单次运行随机种子')
    parser.add_argument('--multi_seeds', type=str, default='', help='多个seed，逗号分隔，例如: 11,22,33')
    parser.add_argument('--max_train_episodes', type=int, default=2000, help='每个场景最大训练轮数')
    parser.add_argument('--eval_episodes', type=int, default=30, help='每个场景评估episode次数')
    parser.add_argument('--stochastic_eval', action='store_true', help='评估时使用随机采样动作（默认确定性argmax）')
    ...
    if args.multi_seeds.strip():
        run_seeds = [int(x.strip()) for x in args.multi_seeds.split(',') if x.strip()]
    else:
    ...
    set_global_seed(run_seeds[0], deterministic=True)

    print("开始深度强化学习数值实验")
    print(f"实验时间: {datetime.now()}")
    print(f"PyTorch版本: {torch.__version__}")
    print(f"设备: {'CUDA' if torch.cuda.is_available() else 'CPU'}")
    print(f"运行seeds: {run_seeds}")
    print(f"评估模式: {'确定性(argmax)' if deterministic_eval else '随机采样'}")
    ...
    for run_seed in run_seeds:
        print(f"\n开始并行训练各个场景的DRL智能体... 当前seed={run_seed}")
        ...
        num_cores = min(mp.cpu_count(), num_scenarios)
        print(f"使用 {num_cores} 个进程进行并行训练")

        with mp.Pool(processes=num_cores) as pool:
            results = pool.starmap(train_scenario, pool_args)

        print(f"\nseed={run_seed} 所有场景训练完成！结果汇总：")
        for scenario_id, reward, eval_stats in sorted(results):
            if reward is None or eval_stats is None:
                print(f"场景 {scenario_id}: 训练失败")
            ...
            print(
                f"场景 {scenario_id}: 训练奖励 = {reward:.2f} | "
                f"评估 = {eval_stats['mean']:.2f}±{eval_stats['std']:.2f}"
            )
            all_seed_rows.append({
                'seed': run_seed,
                'scenario_id': scenario_id,
                ...
                'eval_num_episodes': int(eval_stats['num_episodes']),
                'deterministic_eval': deterministic_eval
            })

    if all_seed_rows:
        seed_df = pd.DataFrame(all_seed_rows)
        seed_detail_path = os.path.join(results_dir, 'train_rewards_by_seed.csv')
        seed_df.to_csv(seed_detail_path, index=False)
        ...
        summary_df = (
            ...
            .agg(
                ...
                train_reward_std=('train_reward', 'std'),
                eval_mean_mean=('eval_mean', 'mean'),
                eval_mean_std=('eval_mean', 'std'),
                eval_std_mean=('eval_std', 'mean')
            )
            .sort_values('scenario_id')
        )
        summary_path = os.path.join(results_dir, 'train_rewards_summary.csv')
        summary_df.to_csv(summary_path, index=False)

        print(f"\n已保存多seed明细: {seed_detail_path}")
        print(f"已保存多seed汇总: {summary_path}")
