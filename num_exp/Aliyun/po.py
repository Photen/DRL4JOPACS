    ...
    def get_action(self, period, env_state):
        ...
        if self.learning:
            ...
            if self.p_now_test == len(self.price_list):
                # print("p_now_test == len(self.price_list)")
                self.learning = False
            else:
...
def main():
    ...
    if results:
        ...
        mean_df.columns = ['PO']
        mean_df.index = [f"Scenario_{s}" for s in summary.index] # type: ignore
        ...
        std_df = summary[['std']]
        std_df.columns = ['PO']
        std_df.index = [f"Scenario_{s}" for s in summary.index] # type: ignore
