...
def states_to_tensor(states, env, device):
    obs_list = []
    for state in states:
...
class PPOAgent:
    ...
    def get_action(self, state, env):
        s_tensor = states_to_tensor([state], env, self.device)
        with torch.no_grad():
    ...
    def train(self, env_fn, max_episodes=1000, rollout_len=None, save_path='models', verbose=True):
        dummy_env = env_fn()
        ...
        for episode in range(max_episodes):
            ...
            for t in range(rollout_len):
                s_tensor = states_to_tensor(states, dummy_env, self.device)
            ...
            with torch.no_grad():
                next_value = self.model.forward(states_to_tensor(states, dummy_env, self.device))[1]
